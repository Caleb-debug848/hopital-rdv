...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-card">
        <div class="flex items-center gap-4">
            ...
            <div>
                <div class="flex items-center gap-2">
                    <h1 class="text-xl font-heading font-extrabold text-slate-900">Bonjour, {{ Auth::user()->prenom }} {{ Auth::user()->nom }}</h1>
                </div>
                <div class="flex items-center gap-3 text-xs text-slate-500 mt-1">
                    <span class="font-mono font-semibold bg-slate-100 px-2 py-0.5 rounded text-slate-700">Dossier : {{ $patient->numero_patient }}</span>
                    <span>•</span>
    ...
    @if($prochainRdv)
        <div class="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white rounded-2xl p-6 shadow-md border border-slate-800 relative overflow-hidden">
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ route('patient.rendez-vous.show', $prochainRdv->id) }}" class="px-4 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 text-white font-bold text-xs border border-white/20 transition backdrop-blur-xs flex items-center gap-1.5">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-card flex items-center justify-between">
            <div>
                <div class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">RDV Totaux</div>
                <div class="text-2xl font-heading font-extrabold text-slate-900 mt-1">{{ $stats['total'] }}</div>
            </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-card flex items-center justify-between">
            <div>
                <div class="text-[11px] font-bold text-brand-600 uppercase tracking-wider">À Venir</div>
                <div class="text-2xl font-heading font-extrabold text-brand-600 mt-1">{{ $stats['a_venir'] }}</div>
            </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-card flex items-center justify-between">
            <div>
                <div class="text-[11px] font-bold text-emerald-600 uppercase tracking-wider">Effectués</div>
                <div class="text-2xl font-heading font-extrabold text-emerald-600 mt-1">{{ $stats['effectues'] }}</div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            @if($rdvAvenir->isEmpty())
                <div class="bg-white rounded-2xl p-8 border border-slate-200/80 shadow-card text-center space-y-3">
                    ...
                    <a href="{{ route('patient.rendez-vous.create') }}" class="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-brand-50 hover:bg-brand-100 text-brand-700 text-xs font-bold transition">
            ...
            @else
                <div class="space-y-3">
                    @foreach($rdvAvenir as $rdv)
                        <div class="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-card hover:border-slate-300 transition flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div class="flex items-start gap-4">
                                <div class="w-12 h-12 rounded-xl bg-slate-100 border border-slate-200 text-slate-700 flex flex-col items-center justify-center font-bold text-xs flex-shrink-0">
                                    <span class="text-sm font-extrabold text-slate-900 leading-none">{{ \Carbon\Carbon::parse($rdv->date_rdv)->format('d') }}</span>
                                    <span class="text-[9px] uppercase font-bold text-slate-500 mt-0.5">{{ \Carbon\Carbon::parse($rdv->date_rdv)->translatedFormat('M') }}</span>
                                </div>
                                <div>
                                    <div class="flex items-center gap-2">
                                        <span class="font-bold text-slate-900 text-xs sm:text-sm">{{ $rdv->medecin->nom_complet }}</span>
                            ...
                            <div class="flex items-center gap-2 self-end sm:self-center">
                                <a href="{{ route('patient.rendez-vous.show', $rdv->id) }}" class="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold transition flex items-center gap-1">
